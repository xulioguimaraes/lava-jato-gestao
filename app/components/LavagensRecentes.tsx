...
export function LavagensRecentes({ lavagens }: { lavagens: LavagemComFuncionario[] }) {
  return (
    <div className="card flex flex-col h-full lg:col-span-2">
      <div className="px-4 py-3 border-b border-slate-700 flex justify-between items-center bg-slate-800/50">
        <h3 className="font-semibold text-slate-100 text-sm">Últimas Lavagens</h3>
      ...
      <div className="p-0 flex-1 overflow-hidden">
        {lavagens.length === 0 ? (
          <div className="text-center py-10">
            ...
            <p className="text-slate-400 text-sm">Nenhuma lavagem registrada.</p>
          </div>
        ) : (
            ...
            {lavagens.slice(0, 5).map((lavagem) => (
              ...
              >
                <div className="flex gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-slate-100">{lavagem.descricao}</p>
                    <p className="text-xs text-slate-400 mt-0.5">
                      ...
                      {lavagem.forma_pagamento && (
                        <>
                          {" "}
                          • Forma: {lavagem.forma_pagamento === "pix" ? "Pix" : "Dinheiro"}
                        </>
                      )}
                    </p>
                  </div>
