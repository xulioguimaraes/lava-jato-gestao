...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    return json({ erro: "Erro ao criar funcionário. Tente novamente." }, { status: 500 });
  }
}
      ...
      <header className="bg-slate-800 border-b border-slate-700">
        <div className="max-w-4xl mx-auto px-3 sm:px-4 lg:px-6 py-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-base font-semibold text-slate-100 leading-none">Novo Funcionário</h1>
              <p className="text-xs text-slate-400 mt-0.5">Cadastre um novo membro da equipe</p>
            </div>
          </div>
        </div>
      </header>
